...
final class q3FinalHandler implements HttpHandler{
	...
	public q3FinalHandler(HConnection connection) throws IOException,FileNotFoundException{
		...
		System.out.println("Q3 build cache start");
		...
		System.out.println("Q3 Cache Build success");
	...
	public void handleRequest(HttpServerExchange exchange) throws Exception {
		...
		String userValue = values.peekFirst();
		int userid = (int)(Long.valueOf(userValue) + Integer.MIN_VALUE);
		...
		if(true)
		{
			...
			Get g = new Get(Bytes.toBytes(userValue));
			Result r = table.get(g);
			String Value = Bytes.toString(r.getValue(Bytes.toBytes("cf"),Bytes.toBytes("content")));
			resultString = resultString.append(Value.replace('\u0001', '\n'));
			try{
				table.close();
			}
			catch(IOException e)
